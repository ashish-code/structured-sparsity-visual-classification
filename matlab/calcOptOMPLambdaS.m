% compute optimal lambda for sparse encoding, lambda input
function calcOptOMPLambdaS(dataSet,dictType,dictSize,sampleSize,mode,modeD,lambdas)
...
modeLs = [0,1,2];
%---------------------------------------------------------------------
for iLambda = 1 : max(size(lambdas))
    dictDataFile = [(rootDir),(dataSet),(dictDir),(dataSet),num2str(dictSize),(dictType),'dl','mode',num2str(mode),'modeD',num2str(modeD),'lambda',num2str(lambdas(iLambda)),'.dict'];
    callDictLambda(dictDataFile,sampleData,dataSet,dictSize,mode,modeD,lambdas(iLambda));
    %---------------------------------------------------------------------
    for iModeL = 1 : max(size(modeLs))
        callOMPCoeffNClassify(dictDataFile,dataSet,dictSize,mode,modeD,lambdas(iLambda),modeLs(iModeL));
    end
    %---------------------------------------------------------------------
end
end
...
function callDictLambda(dictDataFile,sampleData,dataSet,dictSize,mode,modeD,lambda)
    ...
    param.modeD = modeD;
    if mode == 3
        param.lambda = ceil(lambda*dictSize);
    else
        param.lambda = lambda;
    end
    param.K = dictSize;
...
function callOMPCoeffNClassify(dictDataFile,dataSet,dictSize,mode,modeD,lambda,modeL)
rootDir = '/vol/vssp/diplecs/ash/Data/';
...
for iCategory = 1 : nCategory
    ...
    for iListSize = 1 : nListSizes
        ...
        for iter = 1 : nListTrainPos
            ...
            Favg = callOMPnClassify(imageName,dict,dictSize,dataSet,lambda,modeL);          
    ...
    fprintf('%s,%d,%d,%d,%f,%f\n',categoryList{iCategory},mode,modeD,modeL,lambda,averagePrecision);
    %---------------------------------------------------------------------
...
function Favg = callOMPnClassify(imageName,dict,dictSize,dataSet,lambda,modeL)
    rootDir = '/vol/vssp/diplecs/ash/Data/';
    dsiftDir = '/DSIFT/';
