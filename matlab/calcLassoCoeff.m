...
function calcLassoCoeff(dataSet,dictType,dictSize,sampleSize,algo,param)
...
function callLasso(imageName,dict,dataSet,dictType,dictSize,sampleSize,algo,param)
    ...
    coeff = full(alpha);   
    Favg = mean(coeff,2);    
    coeffFilePathAvg = [(rootDir),(dataSet),(coeffDir),imageName,num2str(dictSize),(dictType),num2str(sampleSize),(algo),num2str(param),(method),'.avg'];
