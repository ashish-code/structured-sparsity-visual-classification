% compute optimal lambda for sparse encoding, lambdas input
function calcOptLassoLambdaS(dataSet,dictType,dictSize,sampleSize,mode,modeD,lambda)
%---------------------------------------------------------------------
...
fprintf('%s\n',sampleDataFile);
%---------------------------------------------------------------------
%lambda = [1,10,100,500,1000,5000,10000];
modeLs = [0,1,2];
%---------------------------------------------------------------------
for iLambda = 1 : max(size(lambda))
    ...
    for iModeL = 1 : max(size(modeLs))
        callLassoCoeffNClassify(dictDataFile,dataSet,dictSize,mode,modeD,lambda(iLambda),modeLs(iModeL));
    end
    %---------------------------------------------------------------------
end
end
...
function callLassoCoeffNClassify(dictDataFile,dataSet,dictSize,mode,modeD,lambda,modeL)
rootDir = '/vol/vssp/diplecs/ash/Data/';
...
for iCategory = 1 : nCategory
    ...
    for iListSize = 1 : nListSizes
        ...
        for iter = 1 : nListTrainPos
            ...
            Favg = callLassonClassify(imageName,dict,dataSet,lambda,modeL);          
...
function Favg = callLassonClassify(imageName,dict,dataSet,lambda,modeL)
    rootDir = '/vol/vssp/diplecs/ash/Data/';
    dsiftDir = '/DSIFT/';
