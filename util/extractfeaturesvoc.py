...
def main():
	datasets=['VOC2006','VOC2007','VOC2009','VOC2010']
	rootDir='/vol/vssp/diplecs/ash/Data/'
	progDir = '/vol/vssp/diplecs/ash/code/descriptorvgg/'
	prog = 'extract_features.ln'
	
	for dataset in datasets:
		dataDir = rootDir+dataset+'/'+'ImagesPNG'+'/'
		featureDir = rootDir+dataset+'/'+'Feature'+'/'
		imglist = os.listdir(dataDir)
		os.chdir(dataDir)
		for img in imglist:
			if(img.split('.')[1]=='png'):
				featureName = img.split('.')[0]+'.sift'
				featureFile = featureDir+featureName
				com = progDir+prog+' -harlap -sift'+' -i '+img+' -o1 '+featureFile
				os.system(com)
				print(dataset + ' : '+featureName)
				paramfilename = featureDir+img+'.params'
