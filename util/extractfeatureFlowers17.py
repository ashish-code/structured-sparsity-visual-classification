...
def extractFlowers17():
    ...
    for iCategory,categoryName in enumerate(catList):
        ...
        for catImageId in catImageIds:
            catImageName = rootDir+dataSet+dataDir+'image_%04d.png'%(catImageId)
            featureFileName = rootDir+dataSet+featureDir+categoryName+'/'+'%04d.sift'%(catImageId)
            paramFileName = rootDir+dataSet+featureDir+categoryName+'/'+'%04d.sift.params'%(catImageId)
            oldparamFileName = rootDir+dataSet+featureDir+categoryName+'/'+'%04d.params'%(catImageId)
            print catImageName
